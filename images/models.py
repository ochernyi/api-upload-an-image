...
from api_upload_an_image.storage import GoogleCloudMediaFileStorage
...
class Image(models.Model):
    ...
    def get_thumbnails(self):
        ...
        if user_account_tier.can_generate_expiring_links and hasattr(
            self, "expiring_link"
        ):
...
class ExpiringLink(models.Model):
    ...
    def __repr__(self):
        return """ExpiringLink(image='%s', link='%s', expires_in='%s')""" % (
            self.image,
            self.link,
            self.expires_in,
        )
