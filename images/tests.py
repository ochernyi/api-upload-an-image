...
def create_temporary_image():
    ...
    test_image_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "test-assets", "dog.jpeg"
    )
    ...
    return InMemoryUploadedFile(
        image_file, None, "dog.jpg", "image/jpeg", image_file.tell, None
    )
...
class ImageModelTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.expected_image_name = "dog"
        cls.expected_ext = ".jpg"
        cls.expected_image_path = cls.image.get_original_url
    ...
    def test_image_str_method(self):
        expected_str = (
            f"{str(self.user)} - {self.expected_image_path.rsplit('/media/')[1]}"
        )
        self.assertEqual(str(self.image), expected_str)

    def test_image_repr_method(self):
        expected_repr = f"""Image(id='{self.image.id}', 
        user='{self.user}', image='{self.expected_image_path.rsplit('/media/')[1]}', 
        uploaded_at='{self.image.uploaded_at}')"""
        self.assertEqual(repr(self.image), expected_repr)

    def test_get_original_url_method(self):
        self.assertEqual(self.image.get_original_url, self.expected_image_path)

    def test_get_thumbnails_method(self):
        user_account_tier = self.user.account_tier
        sizes = user_account_tier.get_available_heights
        exp_str = f"/media/{self.user.id}/images/{self.image.id}/{self.expected_image_name}_%s{self.expected_ext}"
