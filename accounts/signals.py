...
def create_default_account_tiers(sender, **kwargs):
    if sender.name == UserAccount._meta.app_label:
        ...
        basic_tier = AccountTier.objects.create(
            ...
            can_generate_expiring_links=DEFAULT_TIERS_CONFIG["BASIC"][
                "can_generate_expiring_links"
            ],
        )
        basic_thumbnail = ThumbnailSize.objects.create(
            **DEFAULT_TIERS_CONFIG["BASIC"]["thumbnail_size"]
        )
        ...
        premium_tier = AccountTier.objects.create(
            ...
            can_generate_expiring_links=DEFAULT_TIERS_CONFIG["PREMIUM"][
                "can_generate_expiring_links"
            ],
        )
        premium_thumbnail = ThumbnailSize.objects.create(
            **DEFAULT_TIERS_CONFIG["PREMIUM"]["thumbnail_size"]
        )
        ...
        enterprise_tier = AccountTier.objects.create(
            ...
            can_generate_expiring_links=DEFAULT_TIERS_CONFIG["ENTERPRISE"][
                "can_generate_expiring_links"
            ],
        )
